...
- Files/components involved:
  ...
  - `.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json` — initial version bump 0.1.3 → 0.2.0 for the new skill, then patch bumps for review hardening (currently 0.2.8)
...
- [x] ➕ preserve and report the displaced active-plan inode after successful publication so a late write through a pre-opened descriptor remains recoverable
...
## Technical Details
- Codex invocation goes through the bundled `run-codex.sh`, which calls `codex exec` non-interactively from a fresh temporary working directory containing only tracked and non-ignored untracked single-link regular files, copied through descriptor-anchored no-follow reads and excluding `.git/`, `.loopai/`, recovery paths, and their case aliases. A restricted permission profile confines reads to that directory and minimal runtime files; the wrapper requires Codex strict-config and permission-profile support, rejects unknown isolation settings, and fails closed when a nonstandard Git private or common directory lives inside the worktree. This prevents repository Codex configuration, link aliases, ignored private artifacts, and stageable recovery data from loading; user configuration, rules, MCP servers, skills, apps, hooks, plugins, and external tools are also disabled; credential-like shell variables are removed; approval escalation is denied; and the session is ephemeral. It never uses `loopai --codex --plan`: loopai's plan creation is interactive and writes files, while the grill needs captive text output.
...
- All plan reads and writes go through the bundled path helper. Active plans are read into an outside-repository snapshot and replaced only when an identity-bound content token confirms the reviewed single-link file is unchanged; publication displaces the reviewed version to a Git-private, non-stageable recovery path on the plan filesystem, reports that path, retains the prior inode for late pre-opened-descriptor writes, and installs the edit with a no-clobber link so a concurrent writer is never overwritten. Plan-off scratch is validated outside the repository. Final plan output is fully written and synced under a hidden name while a shared publication lock serializes alias collision rechecks and atomic no-clobber publication.
