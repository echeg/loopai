...
## Technical Details
- Codex invocation goes through the bundled `run-codex.sh`, which calls `codex exec` non-interactively from a fresh temporary working directory containing only tracked and non-ignored untracked working-tree files, excluding `.git/` and `.loopai/`. A restricted permission profile confines reads to that directory and minimal runtime files. This prevents repository Codex configuration and ignored private artifacts from loading; user configuration, rules, MCP servers, skills, apps, hooks, plugins, and external tools are also disabled; credential-like shell variables are removed; approval escalation is denied; and the session is ephemeral. It never uses `loopai --codex --plan`: loopai's plan creation is interactive and writes files, while the grill needs captive text output.
