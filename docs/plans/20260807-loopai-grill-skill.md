...
- Files/components involved:
  ...
  - `.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json` — initial version bump 0.1.3 → 0.2.0 for the new skill, then patch bumps for review hardening (currently 0.2.7)
...
## Technical Details
- Codex invocation goes through the bundled `run-codex.sh`, which calls `codex exec` non-interactively from a fresh temporary working directory containing only tracked and non-ignored untracked single-link regular files, copied through descriptor-anchored no-follow reads and excluding `.git/` and `.loopai/`. A restricted permission profile confines reads to that directory and minimal runtime files; the wrapper requires Codex strict-config and permission-profile support and rejects unknown isolation settings. This prevents repository Codex configuration, link aliases, and ignored private artifacts from loading; user configuration, rules, MCP servers, skills, apps, hooks, plugins, and external tools are also disabled; credential-like shell variables are removed; approval escalation is denied; and the session is ephemeral. It never uses `loopai --codex --plan`: loopai's plan creation is interactive and writes files, while the grill needs captive text output.
