...
- The shared repository preparation lock serializes classification, fresh creation, and final non-blocking run-lock acquisition. Before a missing target can be created, loopai writes a durable per-target preparation marker in shared Git metadata and clears it only after initialization completes and the run lock is held. If a process dies between `git worktree add` and run-lock acquisition, the next invocation sees the marker under the shared lock, removes the partial checkout, and recreates it instead of auto-resuming incomplete setup. Teardown takes the same shared lock before releasing the run lock and removing the worktree, which keeps the required Windows release-before-delete ordering without exposing an unlocked checkout to a resumer.
