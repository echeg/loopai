...
func TestRunPlanChain(t *testing.T) { //nolint:gocyclo // table-style integration subtests intentionally share setup helpers
	...
	t.Run("rejects unprepared worktree checkpoint after source HEAD advances", func(t *testing.T) {
		dir := setupTestRepo(t)
		plansDir := filepath.Join(dir, "docs", "plans")
		require.NoError(t, os.MkdirAll(plansDir, 0o750))
		plans := []string{filepath.Join(plansDir, "one.md"), filepath.Join(plansDir, "two.md")}
		...
		runGit(t, dir, "add", "docs/plans/one.md", "docs/plans/two.md")
		runGit(t, dir, "commit", "-m", "add unprepared checkpoint plans")
		gitSvc, err := git.NewService(dir, noopLogger())
		require.NoError(t, err)
		initialTip, err := gitSvc.HeadHash()
		require.NoError(t, err)
		initialPlans, err := gitSvc.CapturePlanChainSourceState(plans)
		require.NoError(t, err)
		require.NoError(t, savePlanChainCheckpoint(dir, plans, planChainCheckpoint{
			Mode: string(processor.ModeFull), Worktree: true, InitialTip: initialTip,
			InitialPlanStates: initialPlans,
		}))
		require.NoError(t, os.WriteFile(filepath.Join(dir, "advance.txt"), []byte("advance\n"), 0o600))
		runGit(t, dir, "add", "advance.txt")
		runGit(t, dir, "commit", "-m", "advance source after checkpoint")

		_, _, err = verifiedPlanChainCheckpoint(
			t.Context(), opts{PlanFile: plans[0], PlanFiles: plans}, gitSvc, true, false,
		)
		require.ErrorContains(t, err, "source HEAD changed from saved initial tip")
		require.ErrorContains(t, err, "remove the chain checkpoint")
	})

	t.Run("rejects unprepared worktree checkpoint after plan input changes", func(t *testing.T) {
		dir := setupTestRepo(t)
		plansDir := filepath.Join(dir, "docs", "plans")
		require.NoError(t, os.MkdirAll(plansDir, 0o750))
		plans := []string{filepath.Join(plansDir, "one.md"), filepath.Join(plansDir, "two.md")}
		...
		runGit(t, dir, "add", "docs/plans/one.md", "docs/plans/two.md")
		runGit(t, dir, "commit", "-m", "add plan-input checkpoint plans")
		gitSvc, err := git.NewService(dir, noopLogger())
		require.NoError(t, err)
		initialTip, err := gitSvc.HeadHash()
		require.NoError(t, err)
		...
		require.NoError(t, os.WriteFile(plans[1], []byte("# Changed Plan\n"), 0o600))

		_, _, err = verifiedPlanChainCheckpoint(
			t.Context(), opts{PlanFile: plans[0], PlanFiles: plans}, gitSvc, true, false,
		)
		require.ErrorContains(t, err, "source plan inputs changed since the checkpoint was created")
		require.ErrorContains(t, err, "remove the chain checkpoint")
	})
	...
	t.Run("recovery removes finalized worktree after archive commit", func(t *testing.T) {
		dir := setupTestRepo(t)
		plansDir := filepath.Join(dir, "docs", "plans")
		require.NoError(t, os.MkdirAll(plansDir, 0o750))
		plans := []string{filepath.Join(plansDir, "one.md"), filepath.Join(plansDir, "two.md")}
		...
		runGit(t, dir, "add", "docs/plans/one.md", "docs/plans/two.md")
		runGit(t, dir, "commit", "-m", "add finalized worktree plans")
		gitSvc, err := git.NewService(dir, noopLogger())
		require.NoError(t, err)
		initialTip, err := gitSvc.HeadHash()
		require.NoError(t, err)
		worktreePath, _, err := gitSvc.CreateWorktreeForPlanChainContext(
			t.Context(), plans[0], "", "", plans,
		)
		require.NoError(t, err)
		worktreeSvc, err := gitSvc.OpenWorktree(worktreePath)
		require.NoError(t, err)
		preparedTip, err := gitSvc.BranchHash("one")
		require.NoError(t, err)
		require.NoError(t, worktreeSvc.MovePlanToCompleted(filepath.Join(worktreePath, "docs", "plans", "one.md")))
		archivedTip, err := gitSvc.BranchHash("one")
		require.NoError(t, err)
		o := opts{PlanFile: plans[0], PlanFiles: plans}
		require.NoError(t, savePlanChainCheckpoint(dir, plans, planChainCheckpoint{
			Mode: string(processor.ModeFull), Worktree: true, MovePlanOnCompletion: true,
			InitialTip: initialTip, Active: 1, ActiveStartTip: preparedTip,
			ActivePrepared: true, ActiveFinalizing: true,
		}))
		...
		assert.Equal(t, archivedTip, state.PreviousTip)
		assert.NoDirExists(t, worktreePath)
		worktrees, listErr := gitSvc.Worktrees()
		require.NoError(t, listErr)
		for _, worktree := range worktrees {
			assert.NotEqual(t, "one", worktree.Branch)
		}
	})

	t.Run("recovery recreates finalized worktree after interrupted archive move", func(t *testing.T) {
		dir := setupTestRepo(t)
		plansDir := filepath.Join(dir, "docs", "plans")
		require.NoError(t, os.MkdirAll(plansDir, 0o750))
		plans := []string{filepath.Join(plansDir, "one.md"), filepath.Join(plansDir, "two.md")}
		...
		runGit(t, dir, "add", "docs/plans/one.md", "docs/plans/two.md")
		runGit(t, dir, "commit", "-m", "add interrupted archive plans")
		gitSvc, err := git.NewService(dir, noopLogger())
		require.NoError(t, err)
		initialTip, err := gitSvc.HeadHash()
		require.NoError(t, err)
		...
		preparedTip, err := gitSvc.BranchHash("one")
		require.NoError(t, err)
		require.NoError(t, os.MkdirAll(filepath.Join(worktreePath, "docs", "plans", "completed"), 0o750))
		runGit(t, worktreePath, "mv", "docs/plans/one.md", "docs/plans/completed/one.md")
		o := opts{PlanFile: plans[0], PlanFiles: plans}
		require.NoError(t, savePlanChainCheckpoint(dir, plans, planChainCheckpoint{
			Mode: string(processor.ModeFull), Worktree: true, MovePlanOnCompletion: true,
			InitialTip: initialTip, Active: 1, ActiveStartTip: preparedTip,
		...
		require.NoError(t, err)
		assert.True(t, found)
		assert.Zero(t, state.Completed)
		assert.Equal(t, preparedTip, state.ResumePreparedTip)
		assert.NoDirExists(t, worktreePath)

		recreated, _, err := gitSvc.CreateWorktreeForResumedPlanChainContext(
			t.Context(), plans[0], "", "", preparedTip, plans,
		)
		require.NoError(t, err)
		t.Cleanup(func() { require.NoError(t, gitSvc.RemoveWorktree(recreated)) })
		assert.FileExists(t, filepath.Join(recreated, "docs", "plans", "one.md"))
	})

	t.Run("rejects_advanced_completed_branch_before_resuming_successor", testPlanChainRejectsAdvancedCompletedBranch)
	t.Run("rejects_rewritten_active_branch_during_finalization_recovery", testPlanChainRejectsRewrittenActiveBranch)

	t.Run("recovers_finalized_member_when_archival_is_disabled", testPlanChainRecoveryWithoutArchival)
	t.Run("clears_prepared_resume_tip_after_branch_deletion", testPlanChainClearsDeletedPreparedBranch)
	t.Run("clears_prepared_resume_tip_after_revision_pruned", testPlanChainClearsMissingPreparedRevision)
	t.Run("rejects_rewound_prepared_resume_branch", testPlanChainRejectsRewoundPreparedBranch)
...
func testPlanChainRejectsAdvancedCompletedBranch(t *testing.T) {
	dir := setupTestRepo(t)
	plansDir := filepath.Join(dir, "docs", "plans")
	require.NoError(t, os.MkdirAll(plansDir, 0o750))
	plans := []string{filepath.Join(plansDir, "one.md"), filepath.Join(plansDir, "two.md")}
	...
	runGit(t, dir, "add", "docs/plans/one.md", "docs/plans/two.md")
	runGit(t, dir, "commit", "-m", "add resumable plans")
	runGit(t, dir, "checkout", "-b", "one")
	gitSvc, err := git.NewService(dir, noopLogger())
	require.NoError(t, err)
	savedTip, err := gitSvc.HeadHash()
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(filepath.Join(dir, "follow-up.txt"), []byte("fix\n"), 0o600))
	runGit(t, dir, "add", "follow-up.txt")
	runGit(t, dir, "commit", "-m", "follow up completed plan")
	o := opts{PlanFile: plans[0], PlanFiles: plans}
	require.NoError(t, savePlanChainCheckpoint(dir, plans, planChainCheckpoint{
		Mode: string(processor.ModeFull), Completed: 1, PreviousTip: savedTip,
	}))

	_, _, err = verifiedPlanChainCheckpoint(t.Context(), o, gitSvc, false, false)
...
func testPlanChainRejectsRewrittenActiveBranch(t *testing.T) {
	...
	runGit(t, dir, "add", "docs/plans/one.md", "docs/plans/two.md")
	runGit(t, dir, "commit", "-m", "add recovery plans")
	runGit(t, dir, "checkout", "-b", "one")
	require.NoError(t, os.WriteFile(filepath.Join(dir, "prepared.txt"), []byte("prepared\n"), 0o600))
	runGit(t, dir, "add", "prepared.txt")
	runGit(t, dir, "commit", "-m", "prepare active plan")
	gitSvc, err := git.NewService(dir, noopLogger())
	require.NoError(t, err)
	activeStartTip, err := gitSvc.HeadHash()
	require.NoError(t, err)
	...
	_, _, err = verifiedPlanChainCheckpoint(t.Context(), o, gitSvc, false, false)
	require.ErrorContains(t, err, "no longer contains saved start tip")
	saved, found, loadErr := loadPlanChainCheckpoint(dir, plans, string(processor.ModeFull))
	require.NoError(t, loadErr)
	assert.True(t, found)
	assert.Equal(t, 1, saved.Active, "failed recovery must preserve the active checkpoint")
}

func testPlanChainRecoveryWithoutArchival(t *testing.T) {
	dir := setupTestRepo(t)
	plansDir := filepath.Join(dir, "docs", "plans")
	require.NoError(t, os.MkdirAll(plansDir, 0o750))
	plans := []string{filepath.Join(plansDir, "one.md"), filepath.Join(plansDir, "two.md")}
	...
	runGit(t, dir, "add", "docs/plans/one.md", "docs/plans/two.md")
	runGit(t, dir, "commit", "-m", "add no-archive recovery plans")
	runGit(t, dir, "checkout", "-b", "one")
	gitSvc, err := git.NewService(dir, noopLogger())
	require.NoError(t, err)
	activeStartTip, err := gitSvc.HeadHash()
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(plans[0], []byte("# Plan\n\n- [x] Done\n"), 0o600))
	runGit(t, dir, "add", "docs/plans/one.md")
	runGit(t, dir, "commit", "-m", "complete first plan without archival")
	completedTip, err := gitSvc.HeadHash()
	require.NoError(t, err)
	o := opts{PlanFile: plans[0], PlanFiles: plans}
	require.NoError(t, savePlanChainCheckpoint(dir, plans, planChainCheckpoint{
		Mode: string(processor.ModeFull), Active: 1, ActiveStartTip: activeStartTip,
	...
	err = runPlanChain(t.Context(), o, executePlanRequest{
		Mode: processor.ModeFull, GitSvc: gitSvc,
		Config: &config.Config{MovePlanOnCompletion: false}, OrcaStop: &cleanupHolder{},
	}, nil, nil, io.Discard, func(_ context.Context, gotOpts opts, gotReq executePlanRequest, _ *plan.Selector) error {
		calls++
		assert.Equal(t, plans[1], gotOpts.PlanFile)
		assert.Equal(t, completedTip, gotReq.WorktreeStartRef)
		gotReq.Outcome.succeeded = true
		gotReq.Outcome.branchTip = completedTip
		return nil
	})
	require.NoError(t, err)
	assert.Equal(t, 1, calls)
...
func testPlanChainClearsDeletedPreparedBranch(t *testing.T) {
	dir := setupTestRepo(t)
	plansDir := filepath.Join(dir, "docs", "plans")
	require.NoError(t, os.MkdirAll(plansDir, 0o750))
	plans := []string{filepath.Join(plansDir, "one.md"), filepath.Join(plansDir, "two.md")}
	...
	runGit(t, dir, "add", "docs/plans/one.md", "docs/plans/two.md")
	runGit(t, dir, "commit", "-m", "add prepared resume plans")
	...
	require.NoError(t, savePlanChainCheckpoint(dir, plans, planChainCheckpoint{
		...
		InitialTip: preparedTip, InitialPlanStates: initialPlans,
	...
	assert.True(t, found)
	assert.Equal(t, preparedTip, state.ResumePreparedTip)
	runGit(t, dir, "branch", "-D", "one")

	state, found, err = verifiedPlanChainCheckpoint(t.Context(), o, gitSvc, true, true)
	require.NoError(t, err)
	assert.True(t, found)
	assert.Empty(t, state.ResumePreparedTip)
	saved, savedFound, err := loadPlanChainCheckpoint(dir, plans, string(processor.ModeFull))
	require.NoError(t, err)
	assert.True(t, savedFound)
	assert.Empty(t, saved.ResumePreparedTip)
}

func testPlanChainClearsMissingPreparedRevision(t *testing.T) {
	dir := setupTestRepo(t)
	plansDir := filepath.Join(dir, "docs", "plans")
	require.NoError(t, os.MkdirAll(plansDir, 0o750))
	plans := []string{filepath.Join(plansDir, "one.md"), filepath.Join(plansDir, "two.md")}
	...
	runGit(t, dir, "add", "docs/plans/one.md", "docs/plans/two.md")
	runGit(t, dir, "commit", "-m", "add missing prepared revision plans")
	runGit(t, dir, "branch", "one")
	gitSvc, err := git.NewService(dir, noopLogger())
	require.NoError(t, err)
	initialTip, err := gitSvc.HeadHash()
	...
	require.NoError(t, savePlanChainCheckpoint(dir, plans, planChainCheckpoint{
		Mode: string(processor.ModeFull), Worktree: true, MovePlanOnCompletion: true,
		InitialTip: initialTip, InitialPlanStates: initialPlans, ResumePreparedTip: missingTip,
	}))
...
func testPlanChainRejectsRewoundPreparedBranch(t *testing.T) {
	dir := setupTestRepo(t)
	plansDir := filepath.Join(dir, "docs", "plans")
	require.NoError(t, os.MkdirAll(plansDir, 0o750))
	plans := []string{filepath.Join(plansDir, "one.md"), filepath.Join(plansDir, "two.md")}
	...
	runGit(t, dir, "add", "docs/plans/one.md", "docs/plans/two.md")
	runGit(t, dir, "commit", "-m", "add rewound prepared resume plans")
	runGit(t, dir, "branch", "one")
	gitSvc, err := git.NewService(dir, noopLogger())
	require.NoError(t, err)
	preparedTip, err := gitSvc.BranchHash("one")
	...
	_, _, err = verifiedPlanChainCheckpoint(t.Context(), o, gitSvc, true, true)
	require.ErrorContains(t, err, "no longer contains saved prepared tip")
	saved, found, err := loadPlanChainCheckpoint(dir, plans, string(processor.ModeFull))
	require.NoError(t, err)
	assert.True(t, found)
	assert.Equal(t, preparedTip, saved.ResumePreparedTip)
}
...
func TestCmuxWorkspaceFlagParsing(t *testing.T) {
	tests := []struct {
		name string
		args []string
		want string
		...
		{name: "unset"},
		{name: "bare means always", args: []string{"--cmux-workspace"}, want: "always"},
		{name: "explicit always", args: []string{"--cmux-workspace=always"}, want: "always"},
		{name: "auto", args: []string{"--cmux-workspace=auto"}, want: "auto"},
	}
...
func TestPRFlagParsing(t *testing.T) {
	...
	t.Run("bare flag does not swallow conflicting execution flag", func(t *testing.T) {
		o := parseTestOpts(t, "--pr", "--tasks-only")
		assert.True(t, o.prSet)
		assert.True(t, o.TasksOnly)
		require.Error(t, validateFlags(o))
	})
...
func TestWorktreeIgnoredWarning(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			o:    opts{Worktree: true, Review: true, ExternalOnly: true},
			mode: processor.ModeCodexOnly,
			want: warningPrefix + "--external-only; review modes run in the current checkout and create no branch or worktree",
		},
...
func TestStartT3Reporter(t *testing.T) {
	...
	t.Run("normal run records its checkout", func(t *testing.T) {
		got = nil
		cfg := &config.Config{T3: true, TaskProvider: config.ExecutorCodex}
	...
	t.Run("worktree run records only the branch", func(t *testing.T) {
		got = nil
		req := executePlanRequest{
			Config: &config.Config{T3: true, TaskModel: "claude:opus:high"}, GitSvc: gitSvc, MainGitSvc: mainSvc,
		...
		assert.Equal(t, "opus", got[0].Model)
	})
...
func TestT3LaunchArgs(t *testing.T) {
	...
	assert.Equal(t, []string{"--task-model", "codex:m1", "--review-model", "claude:m2", "--external-reviewers", "codex"},
		t3LaunchArgs(opts{TaskModel: "codex:m1", ReviewModel: "claude:m2", ExternalReviewers: "codex"}))
}
...
func TestExternalReviewLegacyConfigKeysRejected(t *testing.T) {
	tmp := t.TempDir()
	...
	t.Chdir(filepath.Dir(localDir))

	_, err := config.LoadReadOnly(globalDir)
	require.ErrorContains(t, err, "parse local config")
	require.ErrorContains(t, err, "config key external_review_tool was removed; use external_reviewers")

	require.NoError(t, os.WriteFile(filepath.Join(localDir, "config"), []byte("plans_dir = plans\n"), 0o600))
	_, err = config.LoadReadOnly(globalDir)
	require.NoError(t, err)
}
...
// reviewerChainConfig returns a config whose external_reviewers chain is explicitly set.
func reviewerChainConfig(chain string) config.Config {
	return config.Config{ExternalReviewers: chain, ExternalReviewersSet: true}
}
...
func TestResolveExternalReviewSelection(t *testing.T) {
	tests := []struct {
		...
		{name: "claude effort-only keeps opus", cfg: reviewerChainConfig("claude::max"), mode: processor.ModeFull, wantTool: "claude", wantModel: "opus", wantEffort: "max"},
		{name: "codex explicit model and effort", cfg: reviewerChainConfig("codex:gpt-5.6:low"), mode: processor.ModeFull, wantTool: "codex", wantModel: "gpt-5.6", wantEffort: "low"},
		{name: "codex max is dropped with warning signal", cfg: reviewerChainConfig("codex:gpt-5.6:max"), mode: processor.ModeFull, wantTool: "codex", wantModel: "gpt-5.6", wantEffort: "", wantMax: true},
		{name: "explicit empty chain disables review", cfg: reviewerChainConfig(""), mode: processor.ModeFull, wantTool: "none"},
		{name: "custom with model rejected", cfg: reviewerChainConfig("custom:anything"), mode: processor.ModeFull, wantErr: "must not specify a model"},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got, err := resolveExternalReviewSelection(&tc.cfg, tc.mode)
			if tc.wantErr != "" {
				require.Error(t, err)
...
func TestResolveExternalReviewerChain(t *testing.T) {
	...
	got, err := resolveExternalReviewSelection(cfg, processor.ModeFull)
	require.NoError(t, err)
	...
	tasksOnly, err := resolveExternalReviewSelection(cfg, processor.ModeTasksOnly)
	require.NoError(t, err)
	...
	genAgents, err := resolveExternalReviewSelection(cfg, processor.ModeGenAgents)
	require.NoError(t, err)
	...
	_, err = resolveExternalReviewSelection(bad, processor.ModeFull)
	require.ErrorContains(t, err, "parse external_reviewers")

	empty := &config.Config{ExternalReviewersSet: true}
	disabled, err := resolveExternalReviewSelection(empty, processor.ModeFull)
	require.NoError(t, err)
	...
	resolved, err := resolveExternalReviewSelection(normalized, processor.ModeFull)
	require.NoError(t, err)
...
func TestRejectRemovedFlags(t *testing.T) {
	const codexMsg = "--codex was removed; set the provider in the model spec instead, e.g. --task-model codex:gpt-6-astra:medium"
	const codexOnlyMsg = "--codex-only was removed; use --external-only instead"
	const genericReviewers = "use --external-reviewers=<provider>[:model[:effort]] instead, e.g. --external-reviewers=codex:gpt-6-astra:high"
	tests := []struct {
		name string
		args []string
		want string
	}{
		{name: "codex", args: []string{"--codex"}, want: codexMsg},
		{name: "codex with a task model", args: []string{"--codex", "--task-model", "gpt-6-astra:medium"}, want: codexMsg},
		{name: "codex only", args: []string{"--codex-only"}, want: codexOnlyMsg},
		{name: "codex and codex only are both reported", args: []string{"--codex", "--codex-only"}, want: codexMsg + "\n" + codexOnlyMsg},
		{
			name: "tool alone",
			args: []string{"--external-review-tool=codex"},
			want: "--external-review-tool was removed; use --external-reviewers=codex instead",
		},
		{
			name: "tool and model fold into one entry",
			args: []string{"--external-review-tool=codex", "--external-review-model=gpt-5.6:high"},
			want: "--external-review-tool and --external-review-model were removed; use --external-reviewers=codex:gpt-5.6:high instead",
		},
		{
			name: "model alone takes its provider from the model name",
			args: []string{"--external-review-model=fable:high"},
			want: "--external-review-model was removed; use --external-reviewers=claude:fable:high instead",
		},
		{
			name: "auto with a recognizable model names that provider",
			args: []string{"--external-review-tool=auto", "--external-review-model=gpt-5.6"},
			want: "--external-review-tool and --external-review-model were removed; use --external-reviewers=codex:gpt-5.6 instead",
		},
		{
			name: "auto alone keeps the automatic reviewer",
			args: []string{"--external-review-tool=auto"},
			want: "--external-review-tool was removed; omit it to keep the automatic reviewer, or name reviewers with --external-reviewers=<provider>[:model[:effort]]",
		},
		{
			name: "none disables through an empty chain",
			args: []string{"--external-review-tool=none", "--external-review-model=opus"},
			want: "--external-review-tool and --external-review-model were removed; disable external review with --external-reviewers=",
		},
		{
			name: "custom carries no model",
			args: []string{"--external-review-tool", "custom"},
			want: "--external-review-tool was removed; use --external-reviewers=custom instead",
		},
		{name: "unknown model has no provider to suggest", args: []string{"--external-review-model", "my-alias"}, want: "--external-review-model was removed; " + genericReviewers},
		{name: "explicit empty model is still a use", args: []string{"--external-review-model="}, want: "--external-review-model was removed; " + genericReviewers},
		{name: "former invalid choice still gets the migration message", args: []string{"--external-review-tool=bogus"}, want: "--external-review-tool was removed; " + genericReviewers},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			o := parseTestOpts(t, tc.args...)
			require.EqualError(t, rejectRemovedFlags(o), tc.want)
			require.EqualError(t, validateFlags(o), tc.want, "validateFlags must surface the migration message first")
		})
	}

	t.Run("no removed flag passes", func(t *testing.T) {
		o := parseTestOpts(t, "--external-only", "--external-reviewers=codex:gpt-5.6:high", "--codex-args=-c x=1")
		require.NoError(t, rejectRemovedFlags(o))
	})
}

func TestRemovedFlagsAreHiddenFromHelp(t *testing.T) {
	var o opts
	parser := flags.NewParser(&o, flags.Default)
	var help bytes.Buffer
	parser.WriteHelp(&help)

	assert.NotRegexp(t, `--codex\s`, help.String())
	for _, removed := range []string{"--codex-only", "--external-review-tool", "--external-review-model"} {
		assert.NotContains(t, help.String(), removed)
	}
	assert.Contains(t, help.String(), "--codex-args", "a live flag sharing the prefix must stay listed")
	assert.Contains(t, help.String(), "--external-reviewers")
}
...
func TestRunRejectsRemovedFlagsBeforeConfigAndDependencies(t *testing.T) {
	...
	t.Chdir(t.TempDir())
	cfgDir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(cfgDir, "config"), []byte("executor = codex\n"), 0o600))

	for _, args := range [][]string{
		{"--codex"},
		{"--codex-only"},
		{"--external-review-tool=claude"},
		{"--external-review-model=opus"},
	} {
		t.Run(args[0], func(t *testing.T) {
			o := parseTestOpts(t, append([]string{"--config-dir", cfgDir, "--no-color"}, args...)...)
			err := run(t.Context(), o)
			require.Error(t, err)
			assert.Contains(t, err.Error(), " removed; ")
			assert.NotContains(t, err.Error(), "config key executor", "config must not be loaded before the flag check")
			assert.NotContains(t, err.Error(), "not found", "dependency checks must not run before the flag check")
		})
	}
}
...
func TestRunHeaderParams(t *testing.T) {
	...
	t.Run("resolved disabled review is recorded", func(t *testing.T) {
		selection, err := resolveExternalReviewSelection(&config.Config{ExternalReviewersSet: true}, processor.ModeFull)
		require.NoError(t, err)
	...
	t.Run("tasks only records disabled review", func(t *testing.T) {
		cfg := reviewerChainConfig("claude")
		selection, err := resolveExternalReviewSelection(&cfg, processor.ModeTasksOnly)
		require.NoError(t, err)
...
func TestStripCmuxWorkspaceArg(t *testing.T) {
	tests := []struct {
		...
		{name: "auto value form", args: []string{"--cmux-workspace=auto", "plan.md"}, want: []string{"plan.md"}},
		{name: "always value form", args: []string{"--cmux-workspace=always", "plan.md"}, want: []string{"plan.md"}},
		{name: "repeated", args: []string{"--cmux-workspace=auto", "--cmux-workspace", "--cmux-workspace=always"}, want: []string{}},
		{
...
func cmuxAutoSpawnStub(t *testing.T, statusOutput string, queryExitCode, spawnExitCode int) string {
	...
	binDir := t.TempDir()
	argvLog := filepath.Join(binDir, "cmux-argv.log")
	writeExecutable(t, filepath.Join(binDir, "cmux"), fmt.Sprintf(`#!/bin/sh
for a in "$@"; do printf '%%s\n' "$a" >> "$CMUX_ARGV_LOG"; done
...
func TestHandOffToCmuxWorkspace(t *testing.T) {
	...
	t.Run("hands off inside cmux", func(t *testing.T) {
		...
		o := opts{CmuxWorkspace: "always", PlanFile: "docs/plans/20260807-my feature.md"}
	...
	t.Run("bare flag remains unconditional", func(t *testing.T) {
		o := parseTestOpts(t, "--cmux-workspace")
		argvLog := cmuxAutoStub(t, "loopai=done in 3h39m icon=bolt\n", 0)
		t.Setenv("CMUX_WORKSPACE_ID", "ws-1")
		chdirRepoRoot(t)

		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		require.True(t, handOffStops(t, o, []string{"--cmux-workspace"}, stdout, stderr))
		assert.Contains(t, stdout.String(), "handed off to cmux workspace")
		assert.Empty(t, stderr.String())
		recorded, readErr := os.ReadFile(argvLog) //nolint:gosec // path built from t.TempDir
		require.NoError(t, readErr)
		assert.NotContains(t, string(recorded), "list-status", "always mode must not query workspace state")
		assert.Contains(t, string(recorded), "new-workspace")
	})

	t.Run("auto hands off when workspace is busy", func(t *testing.T) {
		argvLog := cmuxAutoStub(t, "loopai=external review (gpt-5.6-sol:high) · iteration 2 icon=person.2\n", 0)
		t.Setenv("CMUX_WORKSPACE_ID", "ws-1")
		chdirRepoRoot(t)

		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		o := opts{CmuxWorkspace: "auto"}
		require.True(t, handOffStops(t, o, []string{"--cmux-workspace=auto"}, stdout, stderr))
		assert.Contains(t, stdout.String(), "handed off to cmux workspace")
		assert.Empty(t, stderr.String())
		recorded, readErr := os.ReadFile(argvLog) //nolint:gosec // path built from t.TempDir
		require.NoError(t, readErr)
	...
	for _, tc := range []struct {
		...
		{name: "final pill", status: "loopai=done in 3h39m icon=bolt\n"},
		{name: "no pill", status: "other-agent=working icon=hammer\n"},
	} {
		t.Run("auto stays local with "+tc.name, func(t *testing.T) {
			argvLog := cmuxAutoStub(t, tc.status, 0)
			t.Setenv("CMUX_WORKSPACE_ID", "ws-1")
			chdirRepoRoot(t)

			stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
			assert.False(t, handOffStops(t, opts{CmuxWorkspace: "auto"}, []string{"--cmux-workspace=auto"}, stdout, stderr))
			assert.Empty(t, stdout.String())
			assert.Empty(t, stderr.String())
			recorded, readErr := os.ReadFile(argvLog) //nolint:gosec // path built from t.TempDir
			require.NoError(t, readErr)
	...
	t.Run("auto outside cmux stays local without warning", func(t *testing.T) {
		argvLog := cmuxAutoStub(t, "loopai=task\n", 0)
		t.Setenv("CMUX_WORKSPACE_ID", "")
		chdirRepoRoot(t)

		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		assert.False(t, handOffStops(t, opts{CmuxWorkspace: "auto"}, nil, stdout, stderr))
		assert.Empty(t, stdout.String())
		assert.Empty(t, stderr.String())
	...
	t.Run("auto query failure is debug-only", func(t *testing.T) {
		argvLog := cmuxAutoStub(t, "", 1)
		t.Setenv("CMUX_WORKSPACE_ID", "ws-1")
		chdirRepoRoot(t)
		...
		assert.False(t, handOffStops(t, opts{CmuxWorkspace: "auto", Debug: true}, nil, stdout, stderr))
		assert.Contains(t, stderr.String(), "debug: cmux workspace auto query failed, running here")
		recorded, readErr := os.ReadFile(argvLog) //nolint:gosec // path built from t.TempDir
		require.NoError(t, readErr)
		assert.NotContains(t, string(recorded), "new-workspace")
	})
	...
	t.Run("outside cmux continues locally", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		assert.False(t, handOffStops(t, opts{CmuxWorkspace: "always"}, nil, stdout, stderr))
		assert.Empty(t, stdout.String())
	...
	t.Run("spawn failure continues locally", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		assert.False(t, handOffStops(t, opts{CmuxWorkspace: "always", PlanFile: "p.md"}, nil, stdout, stderr))
		assert.Empty(t, stdout.String())
		assert.Contains(t, stderr.String(), "hand-off failed, running here")
		recorded, readErr := os.ReadFile(argvLog) //nolint:gosec // path built from t.TempDir
		require.NoError(t, readErr)
	...
	t.Run("api key passthrough warns about the fresh environment", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		o := opts{CmuxWorkspace: "always", PlanFile: "p.md", PreserveAnthropicAPIKey: true}
	...
	t.Run("environment-provided options travel with the command", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		o := opts{CmuxWorkspace: "always", PlanFile: "p.md"}
	...
	t.Run("reset preceding a run is handed off", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		o := opts{CmuxWorkspace: "always", Reset: true, PlanFile: "docs/plans/p.md"}
	...
	t.Run("missing plan file continues locally", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		o := opts{CmuxWorkspace: "always", PlanFile: "docs/plans/typo.md"}
	...
	t.Run("plan file that is a directory continues locally", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		o := opts{CmuxWorkspace: "always", PlanFile: "docs/plans"}
	...
	t.Run("unreadable plan file continues locally", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		o := opts{CmuxWorkspace: "always", PlanFile: "docs/plans/p.md"}
	...
	t.Run("symlinked plan file is handed off", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		o := opts{CmuxWorkspace: "always", PlanFile: "docs/plans/link.md"}
	...
	t.Run("plan creation without a plan file is handed off", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		o := opts{CmuxWorkspace: "always", PlanDescription: "add a feature"}
	...
	t.Run("subdirectory of a repository continues locally", func(t *testing.T) {
		...
		o := opts{CmuxWorkspace: "always", ConfigDir: t.TempDir(), PlanFile: "plans/p.md"}
	...
	t.Run("custom vcs backend without a git marker is handed off", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		o := opts{CmuxWorkspace: "always", ConfigDir: configDir, PlanFile: "p.md"}
		require.True(t, handOffStops(t, o, []string{"p.md"}, stdout, stderr))
		assert.Empty(t, stderr.String())
		recorded, readErr := os.ReadFile(argvLog) //nolint:gosec // path built from t.TempDir
		require.NoError(t, readErr)
	...
	t.Run("watch-only serve outside a repository is handed off", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		o := opts{CmuxWorkspace: "always", Serve: true, Watch: []string{watched}, ConfigDir: t.TempDir()}
	...
	t.Run("watch-only serve from configured watch dirs is handed off", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		o := opts{CmuxWorkspace: "always", Serve: true, ConfigDir: configDir}
	...
	t.Run("serve without watch dirs outside a repository continues locally", func(t *testing.T) {
		...
		stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
		o := opts{CmuxWorkspace: "always", Serve: true, ConfigDir: t.TempDir()}
...
func TestRunHandsOffBeforeConfigLoad(t *testing.T) {
	...
	t.Run("successful hand-off exits before config load", func(t *testing.T) {
		...
		require.NoError(t, run(t.Context(), opts{CmuxWorkspace: "always", ConfigDir: badConfigDir, PlanFile: "p.md"}))
		recorded, err := os.ReadFile(argvLog) //nolint:gosec // path built from t.TempDir
		require.NoError(t, err)
		assert.Contains(t, string(recorded), "new-workspace")
	...
	t.Run("interactive plan creation is handed off too", func(t *testing.T) {
		...
		o := opts{CmuxWorkspace: "always", ConfigDir: badConfigDir, PlanDescription: "add a feature"}
	...
	t.Run("failed hand-off continues the normal run", func(t *testing.T) {
		...
		err := run(t.Context(), opts{CmuxWorkspace: "always", ConfigDir: badConfigDir, PlanFile: "p.md"})
		require.ErrorContains(t, err, "load config")
		recorded, readErr := os.ReadFile(argvLog) //nolint:gosec // path built from t.TempDir
		require.NoError(t, readErr)
	...
	t.Run("unresolvable plan file continues the normal run", func(t *testing.T) {
		...
		err := run(t.Context(), opts{CmuxWorkspace: "always", ConfigDir: badConfigDir, PlanFile: "docs/plans/typo.md"})
		require.ErrorContains(t, err, "load config")
		recorded, readErr := os.ReadFile(argvLog) //nolint:gosec // path built from t.TempDir
		require.NoError(t, readErr)
		assert.NotContains(t, string(recorded), "new-workspace",
...
func TestHandOffSucceeded(t *testing.T) {
	tests := []struct {
		...
		{name: "hand-off with no error", o: opts{CmuxWorkspace: "always"}, want: true},
		{name: "hand-off flag with an error", o: opts{CmuxWorkspace: "always"}, earlyErr: errors.New("reset failed")},
...
func TestResolveExternalReviewSelectionValidatesProviders(t *testing.T) {
	for _, tt := range []struct {
		...
		{
			name: "chain codex without model leaves codex default",
			cfg:  config.Config{ExternalReviewersSet: true, ExternalReviewers: "codex, codex::high"},
		},
		{
			name: "chain explicit codex model names entry",
			cfg:  config.Config{ExternalReviewersSet: true, ExternalReviewers: "codex:fable"},
