...
pi CLI wrapper for loopai, allowing pi to replace Claude Code in task and review phases.

## Scripts

### pi-as-claude.sh

Wraps the pi CLI to produce Claude-compatible stream-json output. Acts as a drop-in replacement for `claude` in task and review phases. It translates pi's `--mode json` JSONL event stream into the `content_block_delta` / `result` events that loopai's `ClaudeExecutor` parses. Plan creation mode (`loopai --plan`) has no pi-specific adapter and is untested.

Suppressed events (tool executions, lifecycle noise) are emitted as empty keepalive deltas so a configured `idle_timeout` does not kill a healthy session during a long silent tool run. pi's stderr is re-emitted after the main stream for loopai error/limit pattern detection; any literal `<<<RALPHEX:` token on stderr is neutralized to `<<< RALPHEX:` (a space is inserted) so stray diagnostics cannot be mistaken for a real completion signal.
...
- `PI_MODEL` — model to use (passed as `--model` when loopai does not append a `--model` flag)
- `PI_THINKING` — thinking level (used when loopai does not append an `--effort` flag)
...
Alternatively, disable extensions entirely with `export PI_EXTRA_ARGS="--no-extensions"`, or remove the gating extension while running loopai.

**Model and effort:** loopai appends `--model <m>` / `--effort <e>` per phase. `--model` is forwarded to pi's `--model`; `--effort` maps to pi's `--thinking` (`off`, `minimal`, `low`, `medium`, `high`, `xhigh` pass through; `max` → `xhigh` with a stderr note since pi has no `max` level).
...
pi defaults to the `google` provider and reads its API key from the provider's standard env vars. Set `PI_PROVIDER` to switch providers, and export the matching API key before running loopai.
