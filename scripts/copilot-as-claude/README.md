...
Wraps GitHub Copilot CLI to produce Claude-compatible `stream-json` output, allowing Copilot to replace Claude Code in loopai task, review, and plan phases through the existing `claude_command` / `claude_args` path.

The wrapper emits completed assistant messages rather than token deltas, which keeps task and review output readable in the loopai progress log.
...
Setting `claude_args` to empty is optional. loopai may still pass default Claude flags because of config fallback behavior, and the wrapper ignores unknown flags gracefully.
