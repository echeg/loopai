...
description: >-
  Update peon-ping configuration — volume, pack rotation, categories, active
  pack, and other settings. Use when user wants to change peon-ping settings
  like volume, enable round-robin, add packs to rotation, toggle sound
  categories, or adjust any config.
---
# peon-ping-config
...
The config file is at `/Users/echeg/Projects/IdleMerge/.claude/hooks/peon-ping/config.json`.
...
```bash
bash "/Users/echeg/Projects/IdleMerge/.claude/hooks/peon-ping"/peon.sh packs list
```
