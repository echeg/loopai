...
Invoke Claude only through `${CLAUDE_SKILL_DIR}/scripts/run-claude.sh` and Codex only through `${CLAUDE_SKILL_DIR}/scripts/run-codex.sh`. Never launch a plan-consuming Agent with inherited repository permissions, call `claude -p` or `codex exec` directly, or substitute `loopai --plan` under any `--plan-model`. Both wrappers accept the canonical repository root, a prompt file, and separate stdout and stderr files; create a bounded snapshot containing only tracked and non-ignored untracked single-link regular files; exclude `.git/`, `.loopai/`, recovery paths, ignored files, links, and their case aliases; reject a nonstandard Git private or common directory inside the worktree; and confine model reads to that outside-repository snapshot. The snapshot rejects files over 64 MiB or more than 512 MiB total.
...
Launch these four critics concurrently. Give every critic the full plan content and tell it to return concise findings with evidence, impact, and a proposed plan change. When the plan carries a `## Decision Log`, tell every critic to read it and not to re-raise a point recorded as rejected unless it has new repository evidence that contradicts the recorded reasoning; a point recorded as deferred may be raised again. Apply the same rules to your own verification pass.
...
> Critique this implementation plan adversarially. Find missing tasks, hidden dependencies, underestimated work, invalid repository assumptions, and weak tasks you can refute. Check the actual codebase before asserting a finding. Return concise findings with evidence, impact, and an exact proposed plan change. If the plan contains a `## Decision Log`, do not re-raise a point it records as rejected unless you have new evidence that contradicts the recorded reasoning; a point it records as deferred may be raised again. Do not edit files.
...
6. Record the round in the draft's `## Decision Log` section: one line per finding the user selected as `- <YYYY-MM-DD> grill: **accepted** - <what changed and where>`, one line per verified finding the user declined with a stated reason as `- <YYYY-MM-DD> grill: **rejected** - "<the finding>" - <the user's reason>`, and one line per verified finding the user simply did not select as `- <YYYY-MM-DD> grill: **deferred** - "<the finding>" - not selected this round`. Deferring is not rejecting: only a **rejected** entry suppresses a later re-raise, so never record an unexplained non-selection as rejected. A stated reason is one the user gave in their own words, in the selection response or anywhere in the surrounding conversation; `AskUserQuestion` with `multiSelect` returns a selection and no reason, so when every non-selection is unexplained the round records deferrals only, which is the intended outcome rather than a gap to fill by inventing reasons. When the log already records the same finding as deferred from an earlier round, update that entry's date in place instead of appending a second line, so repeated rounds do not grow the section with near-identical entries. Use the local calendar date and write each entry as a Markdown list item, the form every other writer of this section uses; a bare line appended after an existing bullet renders as a continuation of that bullet instead of a new entry. Create the section immediately before `## Implementation Steps` when it is absent, preserve every existing entry, and never put a checkbox in it, because a checkbox outside a task section makes the plan read as unfinished work and costs extra loop iterations.
7. Re-read the edited draft and confirm its task numbering, checkbox syntax, dependencies, and testing requirements remain coherent, and that the `## Decision Log` carries no checkbox.
8. Run `plan_paths.py replace-active <canonical-repository-root> <validated-path> <recorded-content-token> <temporary-edited-draft>`. The helper moves the exact reviewed version to a Git-private recovery location before publishing with a no-clobber link, so it never overwrites a concurrent writer and recovery data cannot be staged from the worktree; it rejects an alternate Git private directory located inside the worktree. Record both tab-separated paths it returns and report the recovery path to the user; the prior inode remains there so writes through a descriptor opened before publication cannot be lost. Do not remove that recovery file automatically. If the original identity or content changed after the snapshot, report the conflict and leave the current path untouched. If an exceptional race prevents automatic restoration, report the recovery path returned by the helper. Never fall back to Write or Edit on the repository path. After successful publication, follow **Commit the Prepared Plan** below for the validated active-plan path.
...
6. Follow **Commit the Prepared Plan** below for the returned final path. Remove all scratch drafts and judging files, then report the final path, commit hash, and the score table.
...
After successful helper publication of a final plan or selected revisions, commit that plan on the current branch before offering execution or returning the final result. Do this even when execution is postponed, unless the user explicitly asked to leave it uncommitted. Commit once per finished preparation/revision round, not during drafting. This keeps other prepared plans from blocking loopai's clean-checkout checks.

A review with no applied findings makes no commit. Use only the output path already validated and published by `plan_paths.py`; never commit the source of plan-off, snapshots, candidate drafts, or recovery files.

From the repository root, set `PLAN_PATH` to the exact repository-relative output path and inspect its diff; for a new plan inspect a helper-created snapshot, never Read the repository path directly. If that path has no changes, skip the commit. Otherwise run:
