...
6. Record the round in the draft's `## Decision Log` section: one line per finding the user selected as `<YYYY-MM-DD> grill: **accepted** - <what changed and where>`, and one line per verified finding the user did not select as `<YYYY-MM-DD> grill: **rejected** - "<the finding>" - <the user's reason, or that it was not selected>`. Use the local calendar date. Create the section immediately before `## Implementation Steps` when it is absent, preserve every existing entry, and never put a checkbox in it, because the plan parser reads checkboxes as work and one leaking in would become an implementation step.
