...
Map the three keys to `--task-model <v>`, `--review-model <v>`, `--external-reviewers <v>`. Skip a key with no value, and skip a value that does not match `^[A-Za-z0-9._:,+-]+$` - `$loopai-orca` rejects it - naming the skipped key. Join with single spaces.

With Orca available, report the plan path and the launch line `$loopai-orca <plan> <FLAGS>`. When `FLAGS` came out empty, add one line saying no `task_model`/`review_model`/`external_reviewers` is set, so the run would use loopai defaults.
