...
Detect Orca before speaking: `ORCA=${ORCA_CLI_COMMAND:-orca}`, and on Linux outside an Orca terminal use `orca-ide` instead of bare `orca`, which is the GNOME screen reader there. Orca is available when `command -v "$ORCA"` succeeds. Do not call `orca status`; `$loopai-orca` runs its own preflight.
...
Map `executor = codex` to `--codex`, and the other three keys to `--task-model <v>`, `--review-model <v>`, `--external-reviewers <v>`. Skip a key with no value, and skip a value that does not match `^[A-Za-z0-9._:,+-]+$` - `$loopai-orca` rejects it - naming the skipped key. Join with single spaces.

With Orca available, report the plan path and the launch line `$loopai-orca <plan> <FLAGS>`. When `FLAGS` came out empty, add one line saying no `executor`/`task_model`/`review_model`/`external_reviewers` is set, so the run would use loopai defaults.

Then ask how to proceed: run it in Orca now, start implementing here from task 1, or stop. Without Orca, offer the plain `loopai <plan>` run instead.
