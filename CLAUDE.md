...
- **Best-effort by design.** Indication, not functionality: each call gets a 2s timeout, stdout/stderr are discarded, and errors are swallowed without logging — logging them would pollute the progress file. Nothing here can fail a run. `execRunner.run` leaves `cmd.Stdout`/`cmd.Stderr` nil (i.e. `/dev/null`) rather than assigning `io.Discard`: a non-`*os.File` writer makes `os/exec` allocate a pipe plus a copy goroutine that `cmd.Wait` joins, and a grandchild inheriting the write end would hold it open past the context deadline — hanging the execution goroutine `OnPhase` is called from, which is exactly the "cannot fail a run" invariant
...
- **Progress comes from polling the plan file** every 10s in a goroutine started by `Start(ctx)`, counting `plan.TaskStatusDone` against total tasks — so the bar keeps moving during a long task phase without hooking into the phase engines. `Start` reports once up front before the ticker, so the bar is present from the beginning and short runs are covered too; with an empty plan file (plan creation mode) it starts no goroutine at all. A missing/unparsable plan file (it may have moved to `completed/`), zero parsed tasks, or a `done/total` pair unchanged since the last tick skip the tick silently; the goroutine keeps running. `Stop()` is `sync.Once`-guarded and clears the sidebar in a deliberate order: `loadingOff` **first**, then the `stopped` gate, then cancel + join the poller, then `clearStatus`/`clearProgress`. Canceling does not shorten a poll call already in flight (`exec` times out against its own `context.Background()`), so joining first could burn the interrupt handler's entire 2s budget before the spinner clear ever landed — and nothing here expires on its own: cmux puts no TTL on any of the three artifacts, a leftover spinner hangs until cmux restarts, and a leftover progress bar survives even that (cmux persists it in the session snapshot). The status/progress clears go after the join so a tick in flight cannot re-add the bar behind them
- **A phase change after `Stop` cannot resurrect the pill.** `OnPhase` runs on the execution goroutine, which on the force-exit path is still alive while the interrupt handler tears the sidebar down. So `Stop` sets a `stopped` flag before the clears and `OnPhase` returns early once it is set; both take `statusMu` and `OnPhase` holds it across its `set-status` call, so `Stop` waits out an update in flight instead of racing it and a set can never land behind `clearStatus` — with no TTL, such a pill would sit in the tab row for the rest of the cmux session. `Notify` is deliberately **not** gated: a banner is transient state that needs no cleanup, and the plan-mode handoff reports failures through an already-stopped reporter
- **Only `New`, `Start`, `Stop`, `OnPhase`, `Notify`, and `WrapInput` are exported.** The individual sidebar commands (`loadingOn`/`loadingOff`/`setStatus`/`clearStatus`/`setProgress`/`clearProgress`) are internal: driving them from outside would bypass the `stopOnce`/clear ordering `Stop` relies on. There is deliberately no `clearAll` helper — the clears are not one atomic step, they straddle the poller join
...
- `notifyCmuxCompletion` / `cmuxCompletionNotice` own the end-of-run notification for success and failure; both abort routes send nothing (including wrapped) — `ErrUserAborted` for the Ctrl+\ decline, and `context.Canceled` for Ctrl+C, which never becomes `ErrUserAborted` but reaches `executePlan` as `runner: task phase: context canceled`. The decision lives in `cmuxCompletionNotice`, not at the call sites. Plan-mode success is deliberately NOT a completion notice: it sends `plan created` instead, because the implementation run may still follow and the continue prompt is what the user is actually needed for. That notice is raised only after the `planFile == ""` guard — a plan-creation run that produced no file has nothing to announce and does not wait for the user. Failures after the reporter is live are notified too, not just runner failures: the `--serve` dashboard start error in `executePlan`, and the `filepath.Abs` / `CreateBranchForPlan` errors of the plan-mode handoff (the second one through a reporter `Stop` already ran on, which is why `Notify` is not gated). Errors from the delegated `runWithWorktree`/`executePlan` are **not** notified again at the handoff — the downstream funnel owns its own reporting, and notifying on its return value would double-banner every run failure. `runWithWorktree` therefore notifies its **own** setup errors (worktree creation, progress logger, chdir, worktree git service, plan commit): they all return before `executePlan` is entered, so no downstream reporter exists yet to report them — the plan-mode handoff has already called `rep.Stop()` and a direct `--worktree` run never had a reporter. A `handedOff` flag set immediately before the `executePlan` call gates a named-return defer, so exactly one of the two funnels banners any given failure; the notice goes through a locally created `cmux.New` reporter (`Notify` needs no `Start`, and no spinner is raised) with an empty elapsed, which the failure branch of `cmuxCompletionNotice` does not use — the progress log may not exist yet. `branch` is computed at the top of `runWithWorktree` from `EffectiveBranchName` (pure, plan-file derived) so it is available to the banner before the worktree exists
