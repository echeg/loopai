...
func TestAgentLoader_Load_InvalidModelKeepsDescription(t *testing.T) {
	dir := t.TempDir()
	agentsDir := filepath.Join(dir, "agents")
	require.NoError(t, os.MkdirAll(agentsDir, 0o750))
	...
	agents, err := newAgentLoader(defaultsFS).Load("", agentsDir)
	require.NoError(t, err)

	bad := findAgent(agents, "bad")
	require.NotNil(t, bad)
	assert.Empty(t, bad.Model, "invalid model is dropped")
	assert.Empty(t, bad.AgentType, "execution overrides are dropped alongside the invalid model")
	// description decides catalog membership: a model typo must not remove the agent from review
	assert.Equal(t, "reviews sql migrations", bad.Description)
	assert.Contains(t, buf.String(), `[WARN] agent bad: unknown model "gpt-5"`)
}

func TestAgentLoader_Load_NormalizedFrontmatterVariants(t *testing.T) {
	tests := []struct {
		name    string
		content string
	}{
		{"leading blank line", "\n---\ndescription: reviews races\n---\nReview races."},
		{"leading comment line", "# generated by loopai\n---\ndescription: reviews races\n---\nReview races."},
		{"crlf endings", "---\r\ndescription: reviews races\r\n---\r\nReview races.\r\n"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, os.MkdirAll(agentsDir, 0o750))
			require.NoError(t, os.WriteFile(filepath.Join(agentsDir, "dyn.txt"), []byte(tt.content), 0o600))

			agents, err := newAgentLoader(defaultsFS).Load("", agentsDir)
			require.NoError(t, err)

			dyn := findAgent(agents, "dyn")
			require.NotNil(t, dyn)
			assert.Equal(t, "reviews races", dyn.Description)
			assert.Equal(t, "Review races.", dyn.Prompt)

			// the --gen-agents report parses the same raw bytes and must agree with the loader
			reportOpts, _ := ParseAgentOptions(tt.content)
			assert.Equal(t, dyn.Description, reportOpts.Description)
		})
	}
}
