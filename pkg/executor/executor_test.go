...
func TestClaudeExecutor_subagentLine(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			line, throttle := subagentLine(&event)
...
func TestSplitArgs(t *testing.T) {
	tests := []struct {
		...
		{name: "tab separates like a space", input: "-c a=1\t-c b=2", want: []string{"-c", "a=1", "-c", "b=2"}},
		{name: "newline separates like a space", input: "-c a=1\n-c b=2", want: []string{"-c", "a=1", "-c", "b=2"}},
		{name: "only tabs and newlines", input: "\t\n ", want: nil},
		{name: "tab preserved inside quotes", input: "-c k=\"a\tb\"", want: []string{"-c", "k=a\tb"}},
		{name: "backslash escapes tab outside quotes", input: "--cd C:\\\tdir", want: []string{"--cd", "C:\tdir"}},
		{name: "non-breaking space does not separate", input: "-c k=a\u00a0b", want: []string{"-c", "k=a\u00a0b"}},
		{name: "unicode spaces stay inside unquoted token", input: "--cd C:\u2007work", want: []string{"--cd", "C:\u2007work"}},
		{name: "carriage return separates like a space", input: "-c a=1\r-c b=2", want: []string{"-c", "a=1", "-c", "b=2"}},
	...
	tests := []struct {
		...
		},
		{
			...
			env: []string{
				...
				"CLAUDE_PID=123", "CLAUDE_EFFORT=high", "CLAUDE_CODE=1",
				"CLAUDE_CODE_PARENT_SESSION_ID=ghi", "CLAUDE_CODE_SSE_PORT=5000", "HOME=/home/user",
			},
			preserveAPIKey: false,
			want:           []string{"PATH=/usr/bin", "HOME=/home/user"},
		},
