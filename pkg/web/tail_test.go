...
func TestTailer_PhaseFromSection(t *testing.T) {
	tests := []struct {
		...
		{"external claude review", "claude external review iteration 3", status.PhaseExternalReview},
		{"codex evaluates claude", "codex evaluating claude findings", status.PhaseExternalEval},
		{"codex section", "codex analysis", status.PhaseCodex},
		{"legacy codex iteration", "codex iteration 3", status.PhaseCodex},
		{"legacy claude evaluation", "claude evaluating codex findings", status.PhaseClaudeEval},
...
func TestExtractSignalFromText(t *testing.T) {
	tests := []struct {
		...
		{"<<<RALPHEX:EXTERNAL_REVIEW_DONE>>>", "EXTERNAL_REVIEW_DONE"},
		{"ALL_TASKS_DONE", "COMPLETED"},
		{"TASK_FAILED", "FAILED"},
		{"REVIEW_DONE", "REVIEW_DONE"},
		{"EXTERNAL_REVIEW_DONE", "EXTERNAL_REVIEW_DONE"},
